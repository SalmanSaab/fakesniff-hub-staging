...
export function mountUpdates({ compose, feed, ctx }) {
  ...
  let draft = blank();
  /* An unfinished new report, parked while a correction is open. Codex, 5 Sep:
     choosing Correct used to throw away whatever somebody was part-way through
     writing, with no warning and no way back.
     
     It parks the text AND its submission id, as one pair. Correcting a
     different report does not resolve the parked report's own POST, so if that
     POST had already reached the database and only its reply was lost, coming
     back with a fresh id would store a second copy of it. Codex found exactly
     that: two identical cards in the feed. */
  ...
  async function resolveDuplicate(op, attempted) {
    ...
    if (saved) {
      ...
      setBusy(false);
      /* The original POST is now resolved: we have read the row it created. Its
         id belongs to that report and must not be reused. Codex, 5 Sep: holding
         on to it meant the next genuinely separate report of the day collided
         with it and offered to overwrite the morning's work instead of being
         stored. The id is only kept while a POST is still unresolved. */
      submissionId = newId();
  ...
  async function settled(op, okText, staleText) {
    if (!alive || op !== operation) return;
    /* A correction that was opened part-way through writing something else
       hands the unfinished report back rather than binning it, and hands back
       its retry id with it. A new id is minted only when what was just
       confirmed is the composer's own submission. */
    restoreParked();
    editing = null;
    setBusy(false);
    buildCompose();
  ...
  function startEdit(row) {
    ...
    /* Only on the way in from the composer. Moving between two corrections must
       not overwrite the parked report with the correction being left. */
    if (!editing) pendingDraft = { draft: { ...draft }, submissionId };
  ...
  /* Puts a parked report back, text and retry id together, or clears the
     composer and starts a new submission identity when nothing was parked. */
  function restoreParked() {
    if (pendingDraft) {
      draft = pendingDraft.draft;
      submissionId = pendingDraft.submissionId;
      pendingDraft = null;
      return;
    }
    draft = blank();
